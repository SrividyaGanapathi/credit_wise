"""
Layer 0 — Card Discovery Pipeline
===================================
Entry point for the weekly Cloud Run Job.
...
Environment variables:
  ...
  SKIP_ENRICHMENT     Set to "true" to skip step 4 (useful for dry runs)
  SKIP_VALIDATION     Set to "true" to skip step 5
  ENRICHMENT_BATCH    Max cards to enrich per run (default: 50)
  ENABLED_SCRAPERS    Comma-separated scraper source_names to run;
                      leave unset to run all registered scrapers
"""
...
async def main() -> None:
    log.info("══════════════════════════════════════")
    log.info("  CreditWise — Layer 0: Card Discovery")
    log.info("══════════════════════════════════════")
    ...
    if os.getenv("SKIP_VALIDATION", "false").lower() == "true":
        log.info("Step 5/5 — Validation skipped (SKIP_VALIDATION=true)")
    else:
        log.info("Step 5/5 — Validating PDF URLs")
